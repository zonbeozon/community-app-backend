...
public class CommentDeletePermissionEvaluateHandler implements ChannelActionPermissionEvaluateHandler {
    ...
    public void handle(JoinPoint joinPoint) {
        ...
        //요청자거나 요청자보다 권한이 높다면
        if(post.getAuthor().equals(requester) || permissionEvaluator.isSuperiorTo(post.getChannel().getId(), post.getAuthor())) {
            return;
        }
        throw new AccessDeniedException(ErrorCode.ACCESS_DENIED);
    }
    ...
    public boolean isSupport(ChannelAction action) {
        return action == ChannelAction.COMMENT_DELETE;
    }
}
