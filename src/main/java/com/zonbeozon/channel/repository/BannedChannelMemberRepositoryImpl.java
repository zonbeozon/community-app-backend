...
public class BannedChannelMemberRepositoryImpl implements BannedChannelMemberRepositoryCustom {
    ...
    @Override
    public Page<BannedChannelMemberDto> findByChannelId(Long channelId, Pageable pageable) {
        List<BannedChannelMemberDto> content = queryFactory
                .select(Projections.constructor(BannedChannelMemberDto.class,
                        member.id,
                        member.username,
                        new CaseBuilder()
                        ...
                        member.role,
                        bannedChannelMember.reason

                ))
                .from(bannedChannelMember)
                .join(bannedChannelMember.member, member)
        ...
        JPAQuery<Long> countQuery = queryFactory
                .select(bannedChannelMember.count())
                .from(bannedChannelMember)
                .where(bannedChannelMember.channel.id.eq(channelId));
    ...
    private OrderSpecifier<?>[] getOrderSpecifiers(Sort sort) {
        if (sort.isEmpty()) {
            return new OrderSpecifier[]{bannedChannelMember.bannedAt.desc()};
        }
        ...
        return sort.stream()
                .map(order -> {
                    ...
                    PathBuilder<?> pathBuilder = new PathBuilder<>(BannedChannelMember.class, "bannedChannelMember");
