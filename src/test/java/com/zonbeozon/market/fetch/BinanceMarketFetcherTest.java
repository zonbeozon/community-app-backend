...
class BinanceMarketFetcherTest extends AbstractMarketFetcherTest {
    ...
    private static final BinanceMarketFetcher binanceMarketFetcher = new BinanceMarketFetcher(restClientBuilder);
    private static final Market BTC_USDT_BINANCE_MARKET = MarketFixture.BTC_USDT_BINANCE_MARKET;
    private static final Market ETH_USDT_BINANCE_MARKET = MarketFixture.ETH_USDT_BINANCE_MARKET;
    static {
        MarketTestUtils.setId(1L, BTC_USDT_BINANCE_MARKET);
        MarketTestUtils.setId(2L, ETH_USDT_BINANCE_MARKET);
    }
    //MockServer의 paramValue 체크를 위해서는 순서가 보장되어야 되기 때문에 LinkedHashSet사용
    private static final MarketHolder marketHolder = new SimpleMarketHolder(new LinkedHashSet<>(List.of(BTC_USDT_BINANCE_MARKET, ETH_USDT_BINANCE_MARKET)));

    public BinanceMarketFetcherTest() {
        super(binanceMarketFetcher);
    }

    private static List<MarketFetchResponse> createBinanceMarketFetchResponse() {
        return List.of(
                new BinanceMarketFetchResponse("BTCUSDT", BigDecimal.valueOf(120), BigDecimal.valueOf(130),
                        BigDecimal.valueOf(110), BigDecimal.valueOf(125), BigDecimal.valueOf(5),
                        BigDecimal.valueOf(4.2), BigDecimal.valueOf(1000000)),
                new BinanceMarketFetchResponse("ETHUSDT", BigDecimal.valueOf(2200), BigDecimal.valueOf(2300),
    ...
    protected List<MarketFetchData> expectedReturn() {
        return marketFetchDataMapper.map(marketHolder, createBinanceMarketFetchResponse(),FiatType.USD);
    }
}
