...
public class MemberProfileServiceTest extends AbstractIntegrationTest {
    ...
    void shouldNotDeleteImageWhenUpdatingFromSharedResource() {
        ...
        memberProfileService.updateProfile(member.getId(), dummyImage_1.getId());
    ...
    void deletePreviousImageWhenUpdatingFromNonSharedResource() {
        memberProfileService.updateProfile(member.getId(), dummyImage_1.getId());
        memberProfileService.updateProfile(member.getId(), dummyImage_2.getId());

        verify(imageDeleter).deleteImage(eq(dummyImage_1.getId()));
    }
